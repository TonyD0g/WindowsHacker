...
func See_through() {
	...
	//3. 检测物理内存
	//physicalMemory1, _ := physicalMemory1()
	//4. 检测CPU核心数
	//numberOfCPU1, _ := numberOfCPU()
	//5. 检测临时文件数
	numberOfTempFiles1, _ := numberOfTempFiles()
	fmt.Println("output is:"+ strconv.Itoa(numberOfTempFiles1))
	//level := timeSleep1 + bootTime1 + physicalMemory1 + numberOfCPU1 + numberOfTempFiles1 
...
func physicalMemory1() (int, error) {
...
func outputFile(num int) (){
	filePath := strconv.Itoa(num)+".txt"

	// // 要输出的文件路径
	// filePathTrue:= "outputTrue.txt"
	// filePathFalse:= "outputFalse.txt"
	...
	// if num == 1 {
	// 	filePath = filePathTrue
	// }else {
	// 	filePath = filePathFalse
	// }

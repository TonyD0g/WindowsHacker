// CreateProcessAsUser_Test.cpp : 突破session 0 隔离创建用户进程
//WTS开头的函数，可以用于完成服务层与应用层的交互。
...
{
	// 注册服务入口函数,函数传递过程：ServiceMain => DoTask => CreateUserProcess (核心代码)
	//服务程序的入口点与普通程序的入口点不同，需要通过StartServiceCtrlDispatcher函数来设置入口点函数
...
//程序必须注册成为一个系统服务进程，这样才处于session 0 中.
void __stdcall ServiceMain(DWORD dwArgc, char *lpszArgv)
{
...
void DoTask()
{
	...
	// 创建用户桌面进程
	CreateUserProcess("D:\\Coding\\C\\WindowsHacker\\test.exe");
}
...
BOOL CreateUserProcess(char *lpszFileName)
{
	...
	do
	{
		...
		// 复制令牌,令牌模拟级别为SecurityIdentification,令牌类型为TokenPrimary
		// 并无对进程访问令牌进行设置，所以创建出来的桌面进程没有系统权限
